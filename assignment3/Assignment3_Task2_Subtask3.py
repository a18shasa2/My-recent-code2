...
class read_document_class:
  ...
  def IQR_detection(x,y):
    ...
    output_document = pd.read_csv(file_name, sep = '\t')
    ms_data = output_document
    ...
    '''Extract protein row from csv-file.'''    
...
analyse_document = read_document_class
analyse_document.IQR_detection(gene_name,file_name)
analyse_document.outlier_detection(gene_name,file_name)


class LoadedGene:
  def __init__(Gene, genename):
    Gene.genename = genename
    print("\nThe loaded gene is " + Gene.genename)

specific_gene = LoadedGene(gene_name)