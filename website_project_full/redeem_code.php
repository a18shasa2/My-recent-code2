<?php
	...
	if (isset($_POST["logOn"])) {
		...
		$kod = $conn->real_escape_string($_POST["kod"]);
		$data = $conn->query("SELECT id FROM koder WHERE plan=0 AND kod='$kod'");
        //Table "koder" in databse "cartoonwor_databas" has columns called "id", "kod" and "plan". 
        

		if ($data->num_rows > 0) {
            $ERROR = "The code is correct and has now been redeemed! You now have a subscription!";
            $color = "green";
            
            $emails1= $_SESSION["email"];
            
            $sqll22 = "UPDATE users SET plan=2 WHERE email='$emails1'";
            $conn->query ($sqll22);
            
            $sqll223 = "UPDATE koder SET plan=1 WHERE kod='$kod'";
            $conn->query ($sqll223);
			...
			$ERROR = "You have written the code wrong or the code has already been used! Try again!";
            $color = "red";
		}
	}
...
@media(max-width: 500px) {
 ...
 <div id="container" >
     <p><p>
                  <div class="form-wrap">
<!-- a href="profile.php" title="Tillbaka till huvudmenyn" style=text-align:left;color:lightgrey;font-size:12px>← Profile </a -->
            <h2 style="color:white;margin-left:40px">Redeem code &nbsp &nbsp &nbsp &nbsp </h2>
            <form method="post" action="redeem_code.php"> 
            <div class="form-group" style=text-align:left>    
            <label for="password">Code</label> 
              <br>
            <input type="password" name="kod" placeholder="Write your code" />
                </div>
                     
          
         <div class=indent222> 
              <input type="submit" value="Redeem code" name="logOn" class="btn12 btn12-rounded" /> 
              </div>
               <strong>
                 <p style=color:<?php echo $color ?>;text-align:center> <?php echo $ERROR ?>    </p>
                </strong>
            </form>
         </div>
      </div> 
