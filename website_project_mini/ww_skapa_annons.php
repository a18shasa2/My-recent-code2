<?php 
    ...
    if (isset($_POST["register"])) {
        ...
        if(isset($_FILES['image'])){
              ...
              if ($file_name == "") { 
              $file_path = "winwin_annons_box.png";
              }
            ...
            $file_path_datacheck = $connection->query("SELECT plan FROM winwinproduct WHERE product_image='$file_path'");
		...
		//$dataq = $connection->query("INSERT INTO winwinproduct_se (firstName, lastName, email, password, plan, regDate) VALUES ('$firstName', '$lastName', '$email', '$password', '$plan', '$date_current')");_____

           if ($file_path_datacheck->num_rows == 0 || $file_path == "winwin_annons_box.png") {
        ...
        $data = $connection->query("INSERT INTO winwinproduct (product_name, product_reward, product_image, product_desc, plan, email) VALUES ('$titel', '$utbyte', '$file_path', '$forklaring', '$plan', '$email')");
        
        $connection->query("UPDATE winwinaccount SET plan = plan + 1 WHERE email='$email'");
    	...
    	if ($data === false)
           ...
           header("Location: skapat_annons.php");
		   exit();
	}
...
<!-- Beg. of newsarticle -->
 <div id="container">
     <p><p>
         <div class="form-wrap">
            <h2 style="text-align:center">Skapa annons</h2>
            <form method="post" action="winwin_skapa_annons.php" enctype = "multipart/form-data"> 
            <div class="form-group">    
            <label for="first-name">Titel</label>    
            <input type="text" name="titel" placeholder="Skriv in titeln"  />
            </div>
                
            <div class="form-group">    
            <label for="last-name">Förklaring</label>
            <textarea class="form-control" style=border-color:black; name="forklaring" id="body" placeholder="Skriv in förklaringen" cols="30" rows="5"></textarea>
            </div>   
                
            <div class="form-group">    
            <label for="last-name">Utbyte</label>
            <textarea class="form-control" style=border-color:black; name="utbyte" id="body" placeholder="Skriv in utbytet" cols="30" rows="5"></textarea>
            </div>
             
            <div class="form-group">    
            <label for="image">Ladda upp bild (valfritt)</label>    
            <input type = "file" name="image" />
            </div>  

            <div class="indent2">
                <input type="submit" name="register" value="Skapa annons" class="button button--cta button--menu" /> 
            </div>
  ...
  <footer class="pt-4 my-md-5 pt-md-5 border-top">
    <div class="row" >
      <div class="col-12 col-md">
        <img class="mb-2" src="logo.png" alt="" height="78" width="110"><br>© HorseNode™ 2024
      </div>
      ...
      <div class="col-6 col-md">
        ...
        <ul class="list-unstyled text-small">
		  ...
		  <li><div class="text-muted" href="#">NUMBER0</div></li>
        </ul>
      </div>
    </div>
  </footer>
